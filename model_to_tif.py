...
def load_ckpt_weight_to_model(task_conditions_dict, 
                      checkpoint_dir
                      ):
...
def predict_site(site_id, 
    ...
    model, lowest_val_ckpt_fp = load_ckpt_weight_to_model(
        task_conditions_dict, 
        checkpoint_dir
    )
...
def make_model_output_dirs(exp_name, seed, split_number):
    ...
    out_dir_split = os.path.join(out_dir, exp_name, f'seed_{split_seed}_split_{split_number}')
    for path_name in [out_dir, out_dir_exp, out_dir_split]:
        if not os.path.exists(path_name): os.mkdir(path_name)
        
    return out_dir_split

def run_experiment_models_through_one_split(
    test_sites, 
    checkpoint_dir,
    out_dir_split,
    version_id, 
    ...
    for site_id in test_sites:
        output_dir = os.path.join(out_dir_split, site_id)
        if not os.path.exists(output_dir): os.mkdir(output_dir)
        output_fp = os.path.join(output_dir, f'preds_{site_id}_{version_id}.tif')
...
if __name__ == "__main__":
    ...
    for split_number in range(1):
        ...
        checkpoint_dir = os.path.join(exp_root_dir,  'models', version_id)
        ...
        run_experiment_models_through_one_split(
            test_sites,
            checkpoint_dir,
            out_dir_split,
            version_id, 
