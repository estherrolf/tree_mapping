...
def make_splits(num_sites, seed = 0):
    ...
    splits = [{'train': np.hstack((sets[1], sets[2])), 'val': np.array(sets[3]), 'test': np.array(sets[0])},
...
def get_site_splits(random_seed, 
    ...
    all_sites = np.sort(os.listdir(f'{data_dir}/int/lidar/lidar_by_site_32736_10m'))
    ...
    for split_number, split_order_this in enumerate(split_orders):
        ...
        this_split = {'train_sites': train_sites,
                      'val_sites': val_sites,
