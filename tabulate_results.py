...
project_dir = '../../../tambe_lab/Everyone/tree_mapping'
reference_maps_dir = f'{project_dir}/existing_reference_data'
outputs_dir = f'{project_dir}/model_output'
...
models_subset_train_sites = ['random_forest/4_channels',
                             'random_forest/12_channels',
                             'local_only_models/128_filters/12_channels',
                             'finetune_xceptionS2/pretrained/3_layers_tuned']
...
def save_results_as_JSON():
    ...
    for model in reference_maps:
        results[model] = {'total': {}, 'split': {}, 'site': {}}
        ...
        labels = []
        preds = []
        reference_map_by_site_dir = f'{reference_maps_dir}/{model.lower()}_maps_per_site_{resolution}m'
        ...
        for i in range(num_river_intervals):
            ...
            for site in sites:
                ...
                site_river_distances = np.load(f'{feature_dir}/river/distances_to_river_{resolution}m/{site}_distances_to_river_{resolution}m.npy').ravel()
        ...
        for i in range(num_geologies):
            ...
            for site in sites:
                ...
                geology_array = np.load(f'{feature_dir}/geology/geology_{resolution}m/{site}_geology_{resolution}m.npy').ravel()
        ...
        results[model]['total'] = results_aggregated
        results[model]['split'] = results_by_split
        results[model]['site'] = results_by_site
        results[model]['height'] = results_by_height
        results[model]['river'] = results_by_river
        results[model]['geology'] = results_by_geology
    ...
    for model in models:
        ...
        for n_train_sites in train_site_counts:
            results[model][n_train_sites] = {}

            for seed in range(10):
                results[model][n_train_sites][seed] = {'total': {}, 'split': {}, 'site': {}}
                ...
                labels = []
                preds = []
                model_output_dir = f'{outputs_dir}/{model}/{n_train_sites}_train_sites/seed_{seed}'
                ...
                results[model][n_train_sites][seed]['total'] = results_aggregated
                results[model][n_train_sites][seed]['split'] = results_by_split
                results[model][n_train_sites][seed]['site'] = results_by_site
                results[model][n_train_sites][seed]['height'] = results_by_height
                results[model][n_train_sites][seed]['river'] = results_by_river
                results[model][n_train_sites][seed]['geology'] = results_by_geology
    ...
    for setting in reference_maps:
        ...
        for stratifier in stratifiers:
            results_dict[setting][stratifier] = {}

            if stratifier == 'total':
                for metric in ['r2', 'mae', 'mse', 'rmse']:
                    results_dict[setting][stratifier][metric] = float(results[setting][stratifier][metric])
            elif stratifier == 'split':
                for split in results[setting][stratifier]:
                    results_dict[setting][stratifier][split] = {}
                    
                    for metric in ['r2', 'mae', 'mse', 'rmse']:
                        results_dict[setting][stratifier][split][metric] = float(results[setting][stratifier][split][metric])
            elif stratifier == 'site':
                for site in results[setting][stratifier]:
                    results_dict[setting][stratifier][site] = {}
                    
                    for metric in ['r2', 'mae', 'mse', 'rmse']:
                        results_dict[setting][stratifier][site][metric] = float(results[setting][stratifier][site][metric])
            elif stratifier == 'height':
                for interval in results[setting][stratifier]:
                    results_dict[setting][stratifier][interval] = {}

                    for metric in ['perc_10', 'q1', 'median', 'q3', 'perc_90']:
                        results_dict[setting][stratifier][interval][metric] = float(results[setting][stratifier][interval][metric])
                ...
                for category in results[setting][stratifier]:
                    results_dict[setting][stratifier][category] = {}

                    for metric in ['perc_10', 'q1', 'median', 'q3', 'perc_90']:
                        results_dict[setting][stratifier][category][metric] = float(results[setting][stratifier][category][metric])
    ...
    for setting in models:
        ...
        for n_train_sites in train_site_counts:
            results_dict[setting][f'{n_train_sites} train sites'] = {}

            for seed in range(10):
                results_dict[setting][f'{n_train_sites} train sites'][f'seed {seed}'] = {}

                stratifiers = ['total', 'split', 'site', 'height', 'river', 'geology']

                for stratifier in stratifiers:
                    results_dict[setting][f'{n_train_sites} train sites'][f'seed {seed}'][stratifier] = {}

                    if stratifier == 'total':
                        for metric in ['r2', 'mae', 'mse', 'rmse']:
                            results_dict[setting][f'{n_train_sites} train sites'][f'seed {seed}'][stratifier][metric] = float(results[setting][n_train_sites][seed][stratifier][metric])
                    elif stratifier == 'split':
                        for split in results[setting][n_train_sites][seed][stratifier]:
                            results_dict[setting][f'{n_train_sites} train sites'][f'seed {seed}'][stratifier][split] = {}
                            
                            for metric in ['r2', 'mae', 'mse', 'rmse']:
                                results_dict[setting][f'{n_train_sites} train sites'][f'seed {seed}'][stratifier][split][metric] = float(results[setting][n_train_sites][seed][stratifier][split][metric])
                    elif stratifier == 'site':
                        for site in results[setting][n_train_sites][seed][stratifier]:
                            results_dict[setting][f'{n_train_sites} train sites'][f'seed {seed}'][stratifier][site] = {}
                            
                            for metric in ['r2', 'mae', 'mse', 'rmse']:
                                results_dict[setting][f'{n_train_sites} train sites'][f'seed {seed}'][stratifier][site][metric] = float(results[setting][n_train_sites][seed][stratifier][site][metric])
                    elif stratifier == 'height':
                        for interval in results[setting][n_train_sites][seed][stratifier]:
                            results_dict[setting][f'{n_train_sites} train sites'][f'seed {seed}'][stratifier][interval] = {}

                            for metric in ['perc_10', 'q1', 'median', 'q3', 'perc_90']:
                                results_dict[setting][f'{n_train_sites} train sites'][f'seed {seed}'][stratifier][interval][metric] = float(results[setting][n_train_sites][seed][stratifier][interval][metric])
                    elif stratifier == 'river':
                        for interval in results[setting][n_train_sites][seed][stratifier]:
                            results_dict[setting][f'{n_train_sites} train sites'][f'seed {seed}'][stratifier][interval] = {}
                        ...
                        for category in results[setting][n_train_sites][seed][stratifier]:
                            results_dict[setting][f'{n_train_sites} train sites'][f'seed {seed}'][stratifier][category] = {}
